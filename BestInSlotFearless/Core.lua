...
local E = select(2, ...)
local BestInSlot = LibStub("AceAddon-3.0"):NewAddon("BestInSlotFearless", "AceComm-3.0", "AceHook-3.0", "AceSerializer-3.0", "AceTimer-3.0")
local AceGUI = LibStub("AceGUI-3.0")
local L = LibStub("AceLocale-3.0"):GetLocale("BestInSlotFearless")
...
BestInSlot.Author4 = ("%s%s @ %s"):format("|c"..RAID_CLASS_COLORS.DRUID.colorStr, "Zybaki".."|r",ConvertRGBtoColorString(PLAYER_FACTION_COLORS[1]).."Blackhand-EU|r")
--@non-debug@ 
BestInSlot.version = 20190102083439
--@end-non-debug@
BestInSlot.AlphaVersion = not (GetAddOnMetadata("BestInSlotFearless", "Version"):find("Release") and true or false)
local slashCommands = {}
local defaults = {
  char = {
    ['*'] = { --raidTier
...
function BestInSlot:OnInitialize()
  ...
  SLASH_BESTINSLOT1, SLASH_BESTINSLOT2, SLASH_BESTINSLOT3 = '/bestinslot', '/bis', '/bisf'
...
BestInSlot:RegisterSlashCmd("help", (L["%s - this dialog"]):format("/bis help"), function()
  ...
  DEFAULT_CHAT_FRAME:AddMessage(BestInSlot.colorHighlight..("-"):rep(5)..BestInSlot.colorNormal.."BestInSlotFearless "..L["commands"]..BestInSlot.colorHighlight..("-"):rep(5).."|r")
  BestInSlot:Print(("%s: %s (%s)"):format(GAME_VERSION_LABEL, GetAddOnMetadata("BestInSlotFearless", "Version"), BestInSlot.version))
  for i=1,#orderedList do
